...
async function findRecentActiveBot() {
  try {
    await connectDb();
    Recall.initialize();
    
    // Find very recent meetings (last 24 hours)
    ...
    const artifacts = await db.MeetingArtifact.findAll({
      ...
      include: [{
        ...
        attributes: ["title", "startTime", "recallId"],
      }],
    });

    console.log(`\n🔍 Checking ${artifacts.length} meetings from last 24 hours\n`);
