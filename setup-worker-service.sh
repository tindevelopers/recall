...
if ! railway status &> /dev/null; then
    echo "❌ Project not linked. Run: railway link --project recall-recall"
    exit 1
fi

echo "✅ Project linked: recall-recall"
echo ""

# Check if worker service exists
if railway service recall-worker &> /dev/null; then
    echo "✅ Worker service 'recall-worker' already exists"
    railway service recall-worker
else
    echo "⚠️  Worker service 'recall-worker' not found"
    echo ""
    echo "To create the worker service, run this command in your terminal:"
    echo ""
    echo "   railway add --service recall-worker"
    echo ""
    echo "When prompted:"
    echo "   1. Select 'Empty Service'"
    echo "   2. Enter service name: recall-worker"
